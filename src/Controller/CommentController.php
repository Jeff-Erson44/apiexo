...
class CommentController extends AbstractController
{
    ...
    public function index( Article $article  = null , Request $r, EntityManagerInterface $em, ValidatorInterface $v): Response
    {
            ...
            if(isset($headers['token']) && !empty($headers['token'])){
                ...
                $author = $em->getRepository(User::class)->findUserById($r->get('author'));
                // on crée un nouveau commzntaire 
                    $comment = new Comment();
                    // on spécifie les différents parametre ( ceux qui constitue ta table Comment ) 
                    $comment->setArticle($article); 
                    $comment->setContent($r->get('content')); 
